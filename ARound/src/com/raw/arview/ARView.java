...
public class ARView extends Activity implements SensorEventListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*if (isLocAv == false) {
			...
			ArrayList<PlaceClass> malls;// = (ArrayList<PlaceClass>)
										// getIntent().getExtras().getSerializable("loc");
			double lati  = getIntent().getDoubleExtra("latitude", 0.0),
			longi = getIntent().getDoubleExtra("longitude", 0.0);
			String xx = getIntent().getStringExtra("loc");
			Log.d("!!!", ""+lati+"\n"+longi);

			location1 = new Location("akshay");
			location1.setLatitude(lati);location1.setLongitude(longi);

			Toast.makeText(getApplicationContext(), xx, 1).show();
			...
			radarMarkerView = new RadarMarkerView(this, displayMetrics,
					upperLayerLayout);
			...
			RelativeLayout.LayoutParams relaLayoutParams = new RelativeLayout.LayoutParams(
			...
			addContentView(headerFrameLayout, new FrameLayout.LayoutParams(
					LayoutParams.FILL_PARENT, 44, Gravity.TOP));
			addContentView(upperLayerLayout, upperLayerLayoutParams);

			if (!isInited) {
				dataView = new DataView(ARView.this, xx,this.location1 );
					...
					for (int i = 0; i < dataView.coordinateArray.length; i++) {
						if ((int) event.getX() < dataView.coordinateArray[i][0]
							...
							if ((int) event.getY() <= dataView.coordinateArray[i][1]
								...
								Toast.makeText(
										_context,
										"match Found its " + dataView.places[i],
										Toast.LENGTH_SHORT).show();
								return false;
							}
						}
	...
	public void onLocationChanged(Location loc) {
		/*if (isLocAv == false) {
			this.location.setLatitude(loc.getLatitude()); 
			this.location.setLongitude(loc.getLongitude());
			isLocAv = true;
		}*/
	}
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
		try {
			...
			try {
				...
				supportedSizes = com.raw.utils.Compatibility
				...
				parameters
						.setPreviewSize(
								w
										+ supportedSizes.get(supportedSizes
												.size() - 1).width,
								h
										+ supportedSizes.get(supportedSizes
												.size() - 1).height);
			} catch (Exception ex) {
				parameters.setPreviewSize(arView.screenWidth,
						arView.screenHeight);
			}
...
class RadarMarkerView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (!ARView.dataView.isInited()) {
			ARView.dataView.init(ARView.paintScreen.getWidth(),
					ARView.paintScreen.getHeight(), arView.camera,
					displayMetrics, upperLayoutView);
		}

		ARView.dataView.draw(ARView.paintScreen, ARView.azimuth, ARView.pitch,
				ARView.roll);
	}
}
...
class ResolutionsOrder implements java.util.Comparator<Camera.Size> {
	public int compare(Camera.Size left, Camera.Size right) {
		...
		return Float.compare(left.width + left.height, right.width
				+ right.height);
	}
}