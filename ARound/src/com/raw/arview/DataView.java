...
//import com.ark.evento.R;
import com.raw.utils.Camera;
import com.raw.utils.PaintUtils;
import com.raw.utils.RadarLines;
import com.sackstudios.around.MainActivity;
import com.sackstudios.around.PlaceClass;
import com.sackstudios.around.R;
...
public class DataView {
	...
	String placeListStr;
	//double lati_,longi_;
	 //size = rawArr.length() - 1
	
	double[] latitudes ;//= new double[rawArr.length -1];
	double[] longitudes ;//= new double[rawArr.length -1];
	/*
	...
	//double[] latitudes = new double[] {20.958273,20.987774,20.980514,20.845091,21.127899,20.940383,20.782349};
	//double[] longitudes = new double[] {77.758083,77.757465,77.755609,77.758006,79.056759,77.76665,76.678087};
	//double[] latitudes = new double[] {18.457929, 37.775729, 37.775578, 37.77546, 37.775199, 37.774887, 37.774637, 
			//37.774614, 37.774406, 37.774754, 37.774813, 37.774961, 37.774957, 37.775171, 37.775996, 37.775818, 37.775691, 37.775909};
	//double[] longitudes = new double[] {73.85103, -122.419601, -122.419719, -122.42026, -122.419646, -122.419405, -122.42037, 
			//-122.41934, -122.41886, -122.418785, -122.418581, -122.418868, -122.418064, -122.418884, -122.418898, -122.418305, -122.418895, -122.419161};
	
	//double[] latitudes = new double[] {18.457929, 37.775729, 37.775578}; 
	//37.774614, 37.774406, 37.774754, 37.774813, 37.774961, 37.774957, 37.775171, 37.775996, 37.775818, 37.775691, 37.775909};
	//double[] longitudes = new double[] {73.85103, -122.419601, -122.419719}; 
	//-122.41934, -122.41886, -122.418785, -122.418581, -122.418868, -122.418064, -122.418884, -122.418898, -122.418305, -122.418895, -122.419161};
	...
	String[] places ;//= new String[] {"Phoenix Mall", "Amanora Mall", "Ishanya Mall",
			//"Inorbit Mall","Seasons Mall","Kumar Pacific Mall","Abhiruchi Mall"}; 
	...
	public DataView(Context ctx , String xx, Location loc) {
		this._context = ctx;
		this.placeListStr = xx;
		final_location = new Location("akshay");

		Log.d("!!!!", "data view"+loc.toString());
		this.final_location.setLatitude(loc.getLatitude());
		this.final_location.setLongitude(loc.getLongitude());
	}
	...
	public void init(int widthInit, int heightInit, android.hardware.Camera camera, DisplayMetrics displayMetrics, RelativeLayout rel) {
		try {
			...
			String [] rawArr = placeListStr.split("#");
			Log.d("@@@@raw", rawArr[0]+"");
			latitudes = new double[rawArr.length-1];
			longitudes = new double[rawArr.length-1];
			places = new String[rawArr.length-1];
			
			if(rawArr.length==1){
				String[] rr = rawArr[0].split(";");
				places[0] = rr[0];
				latitudes[0] = Double.parseDouble(rr[1]);
				longitudes[0] = Double.parseDouble(rr[2]);
			}
			
			for(int i =0; i<rawArr.length-1;i++){
				String[] rr = rawArr[i].split(";");
				places[i] = rr[0];
				latitudes[i] = Double.parseDouble(rr[1]);
				longitudes[i] = Double.parseDouble(rr[2]);	
			}
			Log.d("@@@@lati", latitudes[0]+"");
			Log.d("@@@@long", longitudes[0]+"");
			Log.d("@@@@places", places[0]+"");
			...
			for(int i=0;i<latitudes.length;i++){
				layoutParams[i] = new RelativeLayout.LayoutParams(800, 800);
				subjectTextViewParams[i] = new RelativeLayout.LayoutParams(200, 200);
				...
				subjectImageViewParams[i] = new  RelativeLayout.LayoutParams(500, 500);
				...
				locationMarkerView[i] = new RelativeLayout(_context);
				RelativeLayout.LayoutParams prms = new RelativeLayout.LayoutParams(1000, 1000);
				//locationMarkerView[i].setLayoutParams(prms);
				
				
				locationMarkerView[i].setBackgroundResource(R.drawable.abc);
				subjectTextViewParams[i].addRule(RelativeLayout.ALIGN_PARENT_RIGHT, RelativeLayout.TRUE);
				subjectTextViewParams[i].topMargin = 5;
				locationMarkerView[i].setLayoutParams(prms);
				...
				locationTextView[i].setTextSize(12.0f);
...
//							Toast.makeText(_context, " LOCATION NO : "+v.getId(), Toast.LENGTH_SHORT).show();
			...
			bmp = BitmapFactory.decodeResource(_context.getResources(), R.drawable.abc);
			...
			//
			currentLocation.setLatitude(final_location.getLatitude());//(18.621526);
			currentLocation.setLongitude(final_location.getLongitude());//(73.801664);
