...
public class DataView {
	...
	public DataView(Context ctx , ArrayList<MyMall> malls) {
		this._context = ctx;
		this.mallsList = malls;
	}
	...
	public void init(int widthInit, int heightInit, android.hardware.Camera camera, DisplayMetrics displayMetrics, RelativeLayout rel) {
		try {
			...
			for(int i=0;i<latitudes.length;i++){
				...
				locationMarkerView[i].setBackgroundResource(R.drawable.amanora);
...
//							Toast.makeText(_context, " LOCATION NO : "+v.getId(), Toast.LENGTH_SHORT).show();
			...
			bmp = BitmapFactory.decodeResource(_context.getResources(), R.drawable.car);
			...
			currentLocation.setLatitude(20.856938);
			currentLocation.setLongitude(77.749857);
